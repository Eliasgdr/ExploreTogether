...
if (isset($_POST['userID']) && is_numeric($_POST['userID']) && isset($_POST['countryIDs'])) {
    ...
    try {
        ...
        // Delete existing entries for this user in userWishlistCountries table
        $stmt = $conn->prepare("DELETE FROM userWishlistCountries WHERE userID = :userID");
        $stmt->bindParam(':userID', $userID, PDO::PARAM_INT);
        $stmt->execute();

        // Prepare SQL statement to insert new entries into userWishlistCountries table
        $stmt = $conn->prepare("INSERT INTO userWishlistCountries (userID, countryID) VALUES (:userID, :countryID)");
        $stmt->bindParam(':userID', $userID, PDO::PARAM_INT);

        // Loop through each countryID and insert into userWishlistCountries table
        ...
        echo json_encode(array('success' => true, 'status_text' => "Wishlist countries updated successfully.", 'status_code' => 200));
        exit;
